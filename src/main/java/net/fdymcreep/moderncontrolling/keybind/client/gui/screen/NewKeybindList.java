...
public class NewKeybindList extends GuiListExtended implements ITooltipList {
    ...
    public class KeyEntry implements ITooltipListEntry {
        ...
        protected ImageButton btnCreateCloneKeybinding; // toolkit compat
        protected ImageButton btnRemoveCloneKeybinding; // toolkit compat
        ...
        protected KeyEntry(@Nonnull KeyBinding keyBinding) {
            ...
            if (MoCKCompatCheck.moCTCanCampat()) {
                this.keyDesc = ToolkitCompat.getKeyDesc(keyBinding);
            } else {
            ...
            if (MoCKCompatCheck.moCTCanCampat()) {
                this.btnCreateCloneKeybinding = new ImageButton(0, 0, 0, 20, 20, 40, 0, 40, 20, new ResourceLocation(ControllingCore.MODID, "textures/gui/widgets.png"));
                this.btnRemoveCloneKeybinding = new ImageButton(0, 0, 0, 20, 20, 60, 0, 60, 20, new ResourceLocation(ControllingCore.MODID, "textures/gui/widgets.png"));
            }
        ...
        public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY, boolean isSelected, float partialTicks) {
            ...
            if (MoCKCompatCheck.moCTCanCampat() && ToolkitCompat.isClone(this.keybinding)) {
                this.btnCreateCloneKeybinding.x = x + 215;
                this.btnCreateCloneKeybinding.y = y;
                this.btnCreateCloneKeybinding.drawButton(this.parentList.mc, mouseX, mouseY, partialTicks);
                this.btnRemoveCloneKeybinding.x = x + 245;
                this.btnRemoveCloneKeybinding.y = y;
                this.btnRemoveCloneKeybinding.drawButton(this.parentList.mc, mouseX, mouseY, partialTicks);
            } else {
            ...
            if (this.btnChangeKeyBinding.mousePressed(this.parentList.mc, mouseX, mouseY)) {
                ...
                if (MoCKCompatCheck.moCTCanCampat()) {
                    this.parentList.mc.displayGuiScreen(new SettingsScreen(
                            ...
                            this,
                            ToolkitCompat.isClone(this.keybinding) ? "keybindingSetting.clone" : "keybindingSetting.general",
                ...
                return true;
            } else if (MoCKCompatCheck.moCTCanCampat() && ToolkitCompat.isClone(this.keybinding)) {
                if (btnCreateCloneKeybinding.mousePressed(this.parentList.mc, mouseX, mouseY)) {
                    KeyBinding origin = ToolkitCompat.getOrigin(this.keybinding);
                    ToolkitCompat.addCloneKeybinding(
                            this.parentList.mc.gameSettings,
                            ToolkitCompat.createNew(origin)
                    );
                    this.parentList.controlsScreen.search();
                    return true;
                } else if (btnRemoveCloneKeybinding.mousePressed(this.parentList.mc, mouseX, mouseY)) {
                    ToolkitCompat.removeCloneKeybinding(this.parentList.mc.gameSettings, this.keybinding);
                    this.parentList.controlsScreen.search();
                }
                return false;
            } else {
